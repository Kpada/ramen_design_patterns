...
public:
    ...
    void OrderRamen() {
        Execute(std::make_unique<CommandCookRamen>(m_chef));
    }

    void OrderGyoza() {
        Execute(std::make_unique<CommandCookGyoza>(m_chef));
    }
...
private:
    void BusinessLogic() final {
        std::cout <<
            PrinterState::Quote <<
            "We're visiting a ramen restaurant. " <<
