...
public:
    ...
    int GetPriceRamen() const {
        return 1000;
    }

    int GetPriceBeer() const {
        return 500;
    }
};
...
public:
    ...
    int GetPriceSushi() const {
        return 2000;
    }

    int GetPriceSake() const {
        return 800;
    }
};
...
private:
    void Print(const std::string& name, int price) const {
        std::cout <<
            ...
            "Today is Thursday. I'm visiting a " << name <<
            " restaurant to dine without drinks. " <<
...
private:
    void Print(const std::string& name, int price) const {
        std::cout <<
            PrinterState::PlainText <<
            "Today is Friday. I'm visiting a " << name <<
            " restaurant to dine with drinks. " <<
            "The cost of the dinner = " << price << '\n';
    }
};
