...
class Ticket {
public:
    explicit Ticket(std::string name) : m_name(std::move(name)) {
        PrintInfo();
    }
...
private:
    void PrintInfo() const {
        std::cout <<
            PrinterState::PlainText << "Ticket created: " << m_name << '\n';
    }
...
public:
    ...
    bool SelectItem(const std::string& menuItemName) {
        if (IsMenuItemAvailable(menuItemName)) {
            std::cout <<
                PrinterState::PlainText << "Chosen " << menuItemName << '\n';
...
public:
    bool StartCooking(std::unique_ptr<Ticket> ticket) const {
        ...
        std::cout <<
            ...
            "Chef got a ticket and started cooking " <<
            ticket->GetName() << '\n';
