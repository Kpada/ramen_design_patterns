...
public:
    ...
    : SharedState(state.m_county, state.m_city, state.m_postalCode, state.m_adressLine)
    {}
...
public:
    ...
    : m_restaurantName(std::move(restaurantName))
...
public:
    ...
    void Operation(const UniqueState &unique_state) const {
        std::cout <<
            PrinterState::Quote <<
            "FlyWeight operation: shared (" << *m_sharedState.get() <<
...
public:
    explicit FlyWeightFactory(std::initializer_list<SharedState> share_states) {
        for (const SharedState& state: share_states) {
            flyweights.insert(
                std::make_pair(GetSharedKey(state), FlyWeight{state})
            );
        }
    }
    ...
    void PrintFlyWeights() const {
        ...
        */
        std::cout <<
            PrinterState::Quote <<
            "There are " << flyweights.size() << " flyweights and " <<
            FlyWeight::GetNumberOfSharedStates() << " shared states.\n";
...
private:
    void BusinessLogic() final {
        ...
        AddRestaurantToGoogleMap(factory,
            { "Best-Ramen", "Ramen",
        ...
        AddRestaurantToGoogleMap(factory,
            { "Ku-Ramen", "Ramen",
              "Russia", "Moscow", "123456", "Untitled street, 42"
            }
        );
        AddRestaurantToGoogleMap(factory,
            { "Best-Gyoza", "Gyoza",
    ...
    void AddRestaurantToGoogleMap(FlyWeightFactory& factory, const RestaurantInfo& info) {
        std::cout <<
