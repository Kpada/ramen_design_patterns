...
public:
    ...
    int GetPrice() const override {
        return this->getComponent().GetPrice();
    }
...
protected:
    const IFood& getComponent() const {
        return *m_component;
    }
...
private:
    ...
    void PrintFoodDescriptionAndPrice(std::shared_ptr<IFood> food, const std::string& title) {
        std::cout <<
            PrinterState::PlainText <<
            title << ": " << food->GetDescription() <<
